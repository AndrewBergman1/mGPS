...
species_select <- function(x,
  ...
  if (remove_correlated) {
    ...
    correlated <- findCorrelation(cor_matrix, cutoff = 0.7, verbose = TRUE, names = FALSE)
...
meta_abundance <- cbind(initial_columns, meta_abundance[, 44:ncol(meta_abundance)], aggregated_meta_abundance)
